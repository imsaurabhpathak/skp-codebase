...
def schema_to_prompt(fields_chunk):
    prompt = "Generate a fake JSON object based on the following schema fields. Each field has a name and a type. Ensure the output is a valid, syntactically correct JSON object with realistic-looking fake data.\n\nFields:\n"
    for field in fields_chunk:
        prompt += f"- {field['name']}: {field['type']}\n"
    prompt += "\nOutput only the JSON object. Do not include explanations or any other text."
    return prompt
...
# Output each chunk as a list of dicts (ready for your schema_to_prompt function)
